...
- Math Challenge: sidebar panel with grade-appropriate problems (K-2 add/sub, 3-5 mul/div, 6-8 squares/mixed, 9-10 algebra, 11-12/IB factorial/exponents), answer checking, hints, and dual-domain explanations: Turing domain (body — DREAD/IADD/ISUB/BRANCH, numbers and physical addresses) shown in blue, Church domain (mind — LOAD/TPERM/CALL/LAMBDA/RETURN, symbols and permissions) shown in gold, with green bridge note explaining body vs mind, numbers vs symbols, and Ada's 1843 program written in pure symbolic math without compiler/OS/superuser
