...
def run_testbench():
    ...
    imem[0] = encode_ctmm_i(42, 0, int(RV32CapFunct3ArithI.ADDI), 1, int(RV32CapOpcode.ARITHI))
    imem[1] = encode_ctmm_r(0, 1, 1, int(RV32CapFunct3Arith.ADD), 2, int(RV32CapOpcode.ARITH))

    async def testbench(ctx):
        print("=" * 60)
        print("RV32Cap Amaranth Testbench — Design Validation")
        print("=" * 60)
        ...
        addi_corrupt = encode_ctmm_i(
            corrupt_integrity, 0, int(RV32CapFunct3ArithI.ADDI), 14, int(RV32CapOpcode.ARITHI))
        ...
        addi_null_xr11 = encode_ctmm_i(
            null_gt_val, 0, int(RV32CapFunct3ArithI.ADDI), 11, int(RV32CapOpcode.ARITHI))
        ...
        print("\n" + "=" * 60)
        print("RV32Cap Amaranth Testbench — All Tests Complete")
        print("=" * 60)
...
#
# RV32CapMSave and RV32CapMLoad are standalone pipeline units (not yet wired
# into RV32CapCore).  Each test drives one module directly with a small Amaranth
# simulator so the internal FSM states and memory buses are fully observable.

from rv32_cap_amaranth.mload import RV32CapMLoad
from rv32_cap_amaranth.msave import RV32CapMSave
from rv32_cap_amaranth.types import (
...
def test_msave_happy_path():
    """RV32CapMSave writes src_gt to dst.location + index*4 when all checks pass.
...
def test_mload_direct_mode():
    """RV32CapMLoad loads an NS entry into a cap register (direct-mode).
