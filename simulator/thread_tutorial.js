...
<li><strong>GC</strong>: triggered automatically by RETURN \u2014 root set = live Zone \u2460 GTs (CR0\u2013CR11) <em>plus</em> the E-GT word in every live CALL frame on the stack; objects unreachable from those roots are swept and the live set is compacted in place; DR5 is reset to the new frontier; CR5 is not traced (region token, not an object reference)</li>
</ul>
...
<div class="sr-sec-item"><span class="sr-sec-num">7</span><strong>Heap allocation &amp; GC.</strong> Objects are written into the heap via DWRITE using CR9. When freespace is exhausted a <code>FAULT [HEAP_FULL]</code> fires. GC is triggered automatically by RETURN. The root set is two-part: the live GTs in <strong>Zone \u2460 (CR0\u2013CR11)</strong> and the <strong>E-GT word in every live CALL frame</strong> on the stack. CR5 is not traced \u2014 it is a region-coverage token for Zone \u2463, not an object reference. From the root set the collector marks reachable heap objects, sweeps the rest, compacts the live set toward word 17, and resets DR5 to the new allocation frontier. The LIFO stack and Zone \u2460 GTs are read but never modified. The simulator\u2019s <strong>Run GC</strong> button provides a manual trigger for interactive demonstration only.</div>
</div>
